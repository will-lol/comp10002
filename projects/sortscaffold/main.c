...
void fill_sorted(data_t A[], int n) {
  int i;
  for (i = 0; i < n; i++) {
    A[i] = i;
  }
...
void fill_reverse(data_t A[], int n) {
  int i;
  for (i = 0; i < n; i++) {
    A[i] = n - i;
  }
...
void fill_random(data_t A[], int n) {
  int i;
  for (i = 0; i < n; i++) {
    A[i] = i;
  }
...
void do_sort_tests(void sortalg(data_t[], int), int n) {
  data_t *A;
  /* yes, this next one is an array of function pointers! */
  void((*fill[])(data_t[], int)) = {fill_sorted, fill_reverse, fill_same,
                                    fill_random};
  ...
  for (i = 0; i < nfuncs; i++) {
    ...
    printf("\tcomparisons = %4.1f per item, ", (double)comps / n / nruns);
